...
if (isPg) {
  try {
    console.log('🐘 Initializing Supabase PostgreSQL Cloud Database Pool...');
    pool = new Pool({
...
const query = async (sql, params = []) => {
  ...
  return new Promise((resolve) => {
    sqliteDb.all(sql, params, (err, rows) => {
      if (err) {
        console.error('SQLite query notice:', err.message);
        resolve([]);
      } else {
        resolve(rows || []);
      }
...
const get = async (sql, params = []) => {
  ...
  return new Promise((resolve) => {
    sqliteDb.get(sql, params, (err, row) => {
      if (err) {
        console.error('SQLite get notice:', err.message);
        resolve(null);
      } else {
        resolve(row || null);
      }
...
const run = async (sql, params = []) => {
  ...
  return new Promise((resolve) => {
    sqliteDb.run(sql, params, function (err) {
      if (err) {
        console.error('SQLite run notice:', err.message);
  ...
  return new Promise((resolve) => {
    sqliteDb.serialize(async () => {
      try {
        ...
        `);
        console.log('✅ SQLite Fallback Database Schema initialized.');
        resolve();
      } catch (err) {
        console.error('SQLite init notice:', err.message);
        resolve();
      }
    });
  });
